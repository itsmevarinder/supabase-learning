...
export function NewsletterChart({ data, totalCount, trendPercent }: NewsletterChartProps) {
  ...
  const points = data.map((d, i) => ({
    ...
    y: baseline - (d.count / max) * (CHART_HEIGHT - 28),
  }));
  ...
  return (
    <div className="w-full">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-sm text-muted-foreground">New subscribers · last 14 days</p>
      ...
      <svg
        ...
        role="img"
        aria-label="Newsletter subscribers gained per day, last 14 days"
      >
        <defs>
          <linearGradient id="newsletter-bar-fill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="var(--chart-3)" stopOpacity="0.5" />
            <stop offset="100%" stopColor="var(--chart-3)" stopOpacity="0.18" />
          </linearGradient>
          <linearGradient id="newsletter-bar-fill-hover" x1="0" y1="0" x2="0" y2="1">
        ...
        {/* Bars */}
        {data.map((d, i) => {
          const barHeight = (d.count / max) * (CHART_HEIGHT - 28);
          ...
          return (
            <g key={d.date}>
              ...
              <rect
                ...
                rx="2"
                fill={isHovered ? "url(#newsletter-bar-fill-hover)" : "url(#newsletter-bar-fill)"}
        ...
        {hovered !== null && (
          <span>
            ...
            <span className="text-muted-foreground">
              new subscriber{data[hovered].count === 1 ? "" : "s"} on {formatDayLabel(data[hovered].date)}
            </span>
          </span>
        )}
      </div>
